...
class REPO {
    ...
    getGoverned(email) {
        let sql="select b.*,c.nome, c.email,c.picture, d.saldo \
    ...
    updatePicture(userId, picture) {
        let sql="UPDATE KISS_USERS set picture='"+picture+"', lastLogin=now() where id="+userId;
        return new Promise( (fulfill, reject) =>{
            this.conn.query(sql)
                .then(result => fulfill(result))
                .catch( error => reject(error))                
    ...
    deleteTransaction(transactionId) {
        ...
        let sql="DELETE FROM KISS_TRANSAZIONI where id="+transactionId;
        return new Promise( (fulfill, reject) =>{
            this.conn.query(sql)
                .then(result => fulfill(result))
                .catch( error => reject(error))                
